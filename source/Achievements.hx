...
class AttachedAchievement extends FlxSprite {
	...
	public function reloadAchievementImage() {
		if(Achievements.isAchievementUnlocked(tag)) {
			loadGraphic(Paths.themeImage('achievementgrid'), true, 150, 150);
			animation.add('icon', [Achievements.getAchievementIndex(tag)], 0, false, false);
			animation.play('icon');
		} else {
			loadGraphic(Paths.themeImage('lockedachievement'));
		}
		scale.set(0.7, 0.7);
		updateHitbox();
	}
...
class AchievementObject extends FlxSpriteGroup {
	...
	public function new(name:String, ?camera:FlxCamera = null)
	{
		...
		var achievementIcon:FlxSprite = new FlxSprite(achievementBG.x + 10, achievementBG.y + 10).loadGraphic(Paths.themeImage('achievementgrid'), true, 150, 150);
