	...
	**/
	...
	function addChartingUI()
	{
		...
		wavefordropdown = new FlxUIDropDownMenuCustom(waveformEnabled.x + 120, waveformEnabled.y,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(songsparts, true), function(character:String)
		{
			checkForVoices = songsparts[Std.parseInt(character)];
			updateWaveform();
		});
		wavefordropdown.selectedLabel = checkForVoices;
		...
		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x, check_mute_inst.y + 30, null, null, "Mute Player/Global Vocals (in editor)", 100);
	...
	function loadAudioBuffer()
	{
		...
		audioBuffers[2] = null;
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/OppVoices.ogg')))
		{
			audioBuffers[2] = AudioBuffer.fromFile(Paths.modFolders('songs/' + currentSongName + '/OppVoices.ogg'));
			// trace('Custom vocals found');
		}
		else
		{
		#end
			var leVocals:String = Paths.getPath(currentSongName + '/OppVoices.' + Paths.SOUND_EXT, SOUND, 'songs');
			if (OpenFlAssets.exists(leVocals))
			{ // Vanilla voices
	...
	function reloadGridLayer()
	{
		...
		var gridBlack:FlxSprite = new FlxSprite(0,
			gridBG.height / 2).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * Note.ammo[_song.mania] * 2), Std.int(gridBG.height / 2), FlxColor.BLACK);
		gridBlack.alpha = 0.4;
		gridLayer.add(gridBlack);

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * Note.ammo[_song.mania])).makeGraphic(2, Std.int(gridBG.height),
			FlxColor.BLACK);
	...
	function updateWaveform()
	{
		...
		var songPartTrack:Int = 1;
		if (checkForVoices == 'Inst')
			songPartTrack = 0;
		if (checkForVoices == 'Opponent')
			songPartTrack = 2;

		if (!waveformEnabled.checked || audioBuffers[songPartTrack] == null)
		{
			// trace('Epic fail on the waveform lol');
			return;
		}

		var sampleMult:Float = audioBuffers[songPartTrack].sampleRate / 44100;
		...
		if (samplesPerRow < 1)
			samplesPerRow = 1;
		var waveBytes:Bytes = audioBuffers[songPartTrack].data.toBytes();
		...
		while (index < (waveBytes.length - 1))
		{
			...
			if ((index % samplesPerRow) == 0)
			{
				...
				waveformSprite.pixels.fillRect(new Rectangle(Std.int((GRID_SIZE * Note.ammo[_song.mania]) - pixelsMin), drawIndex, pixelsMin + pixelsMax, 1),
					FlxColor.BLUE);
				drawIndex++;
	...
	{
		var healthIconP1:Array<String> = loadHealthIconFromCharacter(_song.player1);
		var healthIconP2:Array<String> = loadHealthIconFromCharacter(_song.player2);

		if (_song.notes[curSection].mustHitSection)
		{
			leftIcon.changeIcon(healthIconP1[0], (healthIconP1[1] == '1'));
			rightIcon.changeIcon(healthIconP2[0], (healthIconP2[1] == '1'));
			if (_song.notes[curSection].gfSection)
				leftIcon.changeIcon('gf');
		}
		else
		{
			rightIcon.changeIcon(healthIconP1[0], (healthIconP1[1] == '1'));
			leftIcon.changeIcon(healthIconP2[0], (healthIconP2[1] == '1'));
			if (_song.notes[curSection].gfSection)
				leftIcon.changeIcon('gf');
		}
	}
	...
	function loadHealthIconFromCharacter(char:String)
	{
		...
		if (json.hasWinIcon)
			...
			winIcon = '0';
		return [json.healthicon, winIcon];
	}
	...
	function loadJson(song:String):Void
	{
		if (CoolUtil.difficulties[PlayState.storyDifficulty] == null)
			CoolUtil.difficulties[PlayState.storyDifficulty] = 'Normal';
